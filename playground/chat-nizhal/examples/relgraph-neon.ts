...
const members = pgTable("rg_members", {
  id: text("id").primaryKey(),
  workspace_id: text("workspace_id").notNull(),
  user_id: text("user_id").notNull(),
});
const projects = pgTable("rg_projects", {
  id: text("id").primaryKey(),
  workspace_id: text("workspace_id").notNull(),
  name: text("name").notNull(),
});
const labels = pgTable("rg_labels", {
  id: text("id").primaryKey(),
  workspace_id: text("workspace_id").notNull(),
  name: text("name").notNull(),
});
...
const relSchema = {
  rg_members: members,
  rg_projects: projects,
  rg_labels: labels,
  rg_issues: issues,
  rg_issue_labels: issueLabels,
  rg_comments: comments,
} as const;
...
const relMutators = defineMutators({
  createProject: defineMutator(
    z.object({ id: sid, workspaceId: sid, name: z.string().min(1) }),
    async ({ tx }, a) => {
  ...
  linkBlocking: defineMutator(
    z.object({ workspaceId: sid, issueId: sid, blockedById: sid }),
    async ({ tx }, a) => {
      await tx.update(issues, { id: a.issueId }).set({ parent_issue_id: a.blockedById });
...
async function makeClient(opts: {
  ...
  userId: string;
  spaces: string[];
  realtime: ReturnType<typeof inProcessRealtime>;
}) {
  ...
  const echo = createNizhalClient({
    ...
    subscribeSource: {
      subscribe: (buckets, onMessage) => opts.realtime.subscribe(buckets, { send: onMessage }),
    },
    bucketsForSyncRule: (rule) => (rule === "space" ? opts.spaces : []),
  });
  const coll = (name: string) =>
    createCollection(
      nizhalCollectionOptions<AnyRow>({
        name,
        syncRule: "space",
        echo,
  ...
  const m = createNizhalMutators({
    collections: {
      ...
      rg_issue_labels: issueLabels,
      rg_comments: comments,
    } as unknown as Record<string, NizhalCollection<object>>,
    echo,
...
async function main() {
  ...
  await sql.unsafe(
    "drop table if exists rg_comments, rg_issue_labels, rg_issues, rg_labels, rg_projects, rg_members cascade",
  );
  ...
  const server = createNizhalServer({
    db: url,
    schema: relSchema,
    mutators: relMutators as MutatorRegistry,
    syncRules: relSyncRules,
  ...
  alice.mutate.createIssueCascade({
    id: "i1",
    workspaceId: W,
    projectId: "p1",
    number: 1,
  ...
  for (const l of cLinks) {
    if (!issueIds.has(l.issue_id as string))
      dangling.push(`issue_label ${l.id} → missing issue ${l.issue_id}`);
    if (!labelIds.has(l.label_id as string))
      dangling.push(`issue_label ${l.id} → missing label ${l.label_id}`);
  }
  for (const cm of cComments) {
    if (!issueIds.has(cm.issue_id as string))
      dangling.push(`comment ${cm.id} → missing issue ${cm.issue_id}`);
    if (cm.parent_comment_id && !commentIds.has(cm.parent_comment_id as string))
      dangling.push(`comment ${cm.id} → missing parent ${cm.parent_comment_id}`);
  }
  for (const is of cIssues) {
    if (is.project_id && !projIds.has(is.project_id as string))
      dangling.push(`issue ${is.id} → missing project ${is.project_id}`);
    if (is.parent_issue_id && !issueIds.has(is.parent_issue_id as string))
      dangling.push(`issue ${is.id} → missing blocker ${is.parent_issue_id}`);
  }
  console.log(
    `  (1) referential integrity: ${dangling.length === 0 ? "✅ no dangling references" : `🔴 ${dangling.length} DANGLING:\n     - ${dangling.join("\n     - ")}`}`,
  );

  // (2) Atomic cascade: i1 present WITH both labels + opening comment (none lost on offline replay).
  const i1Labels = cLinks
    .filter((l) => l.issue_id === "i1")
    .map((l) => l.label_id)
    .sort();
  const i1Comment = cComments.some((c) => c.id === "c1");
  const cascadeOk = issueIds.has("i1") && i1Labels.join(",") === "lbl-bug,lbl-urgent" && i1Comment;
  console.log(
    `  (2) atomic cascade i1: issue=${issueIds.has("i1")} labels=[${i1Labels}] comment=${i1Comment} → ${cascadeOk ? "✅ intact" : "🔴 PARTIAL/LOST"}`,
  );
  ...
  console.log(
    `  (3) issue-number collisions (server truth): ${dbNums.length === 0 ? "✅ none" : `🔴 duplicate numbers: ${JSON.stringify(dbNums)} → two issues share an identifier`}`,
  );

  // (4) No lost write: all three issues converged.
  const allIssues = issueIds.has("i1") && issueIds.has("i2") && issueIds.has("i3");
  console.log(
    `  (4) convergence: all of i1,i2,i3 present on a fresh client → ${allIssues ? "✅" : "🔴 LOST a write"}`,
  );
  ...
  await sql.unsafe(
    `insert into rg_members (id, workspace_id, user_id) values ('m-dave-w', '${W}', 'dave')`,
  );
  daveSpaces.push(W);
  ...
  console.log(
    `  ${daveW.length === 0 ? "🔴 G1-on-a-graph CONFIRMED — joined a workspace, sees an EMPTY tracker (no projects/issues/labels/comments)." : daveW.length < 3 ? "🔴 PARTIAL backfill" : "✅ full graph backfilled"}`,
  );
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        ok: true,
        carolIssues: ids(cIssues),
        carolLabels: ids(cLabels),
        carolLinks: cLinks.length,
        carolComments: ids(cComments),
        dangling,
        cascadeOk,
        numberCollisions: dbNums,
        convergedAll: allIssues,
        g1GraphDaveIssues: ids(daveW),
      },
      null,
      2,
    ),
...
main().catch((e) => {
  ...
  try {
    writeFileSync(RESULT, JSON.stringify({ ok: false, error: String((e as Error)?.stack ?? e) }));
  } catch {}
  process.exit(1);
});
